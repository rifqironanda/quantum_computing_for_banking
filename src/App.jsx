...
const Arrow = ({ reverse = false }) => (
  <span className={`arrow ${reverse ? "reverse" : ""}`} aria-hidden="true">
    →
  </span>
);
...
function Cards({ items }) {
  return <LearningCards items={items} />;
}
      ...
      return (
        <>
          ...
          <div className="intersection">
            ...
            <div className="intersection-center">{slide.center}</div>
                ...
                <div>
                  {g[1].map((x, i) => (
                    <div className="pipe-wrap" key={x}>
                      <span>{x}</span>
                      {(i < g[1].length - 1 ||
                        gi < slide.groups.length - 1) && <Arrow />}
                    </div>
                  ))}
                </div>
              </section>
...
export default function App() {
  ...
  return (
    <main
      className="app"
      onTouchStart={(e) =>
        (touchStart.current = e.target.closest("input,button,a")
          ? null
      ...
      onTouchEnd={(e) => {
        ...
        if (Math.abs(d) > 60) go((i) => i + (d < 0 ? 1 : -1));
      }}
    >
      <div className="topbar">
        ...
        <button onClick={toggleFullscreen} aria-label="Layar penuh">
      ...
      <nav className="controls" aria-label="Navigasi presentasi">
        ...
        <button className="counter" onClick={() => setHelp(true)}>
      ...
      {overview && (
        <div className="overlay" role="dialog" aria-modal="true">
          <div className="overview">
            <header>
              <h2>Daftar slide</h2>
              <button onClick={() => setOverview(false)}>Tutup ×</button>
            </header>
            <div>
              {slides.map((s, i) => (
                <button
      ...
      {help && (
        ...
        >
          <div className="help" onClick={(e) => e.stopPropagation()}>
            <h2>Navigasi</h2>
            <p>
              <kbd>←</kbd> <kbd>→</kbd> pindah slide
            </p>
            <p>
              <kbd>O</kbd> daftar slide
            </p>
            <p>
              <kbd>Home</kbd> / <kbd>End</kbd> awal / akhir
            </p>
            <p>Swipe pada layar sentuh</p>
            <button onClick={() => setHelp(false)}>Mengerti</button>
          </div>
        </div>
      )}
    </main>
